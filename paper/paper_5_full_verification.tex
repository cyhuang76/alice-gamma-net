...
\begin{document}
\newcommand{\ZFP}{zero outcome-fitted parameters}
...
$n = 52{,}545$, $7{,}627$ deaths; zero outcome-fitted parameters; SHA-256
...
zero outcome-fitted parameters) confirms that a vascular measurement
...
\begin{enumerate}
  ...
  \item \textbf{External empirical validation.}
    ...
    with zero outcome-fitted parameters and SHA-256 hash-locking.
...
\begin{enumerate}
  ...
  \item Zero outcome-fitted parameters throughout.%
    \footnote{``Zero outcome-fitted parameters'' means that no
    coefficient in the pipeline was optimised against
    the NHANES outcome labels.
    The reference intervals ($x_{\text{ref},j}$) and
    anatomical weights ($w_j$) are \emph{a~priori}
    clinical consensus constants drawn from textbook values
    (e.g.\ fasting glucose $<100$\,mg/dL,
    SBP $<120$\,mmHg), analogous to using
    $g = 9.81$\,m/s$^2$ in a ballistics prediction.
    They constitute prior physiological knowledge, not
    regression coefficients fitted to the validation cohort.}
\end{enumerate}
...
($n = 7{,}393$, zero outcome-fitted parameters).}
\label{tab:3cycle}
\begin{tabular}{lcc}
\toprule
...
with zero outcome-fitted parameters.
...
zero outcome-fitted parameters, SHA-256 hash-locked protocol),
...
\begin{enumerate}
  \item \textbf{Prediction (blind).}
    ...
    $Z_{\mathrm{normal}}$ (zero outcome-fitted parameters).
...
\caption{Exercise vs.\ $\Sigma\Gamma^2$:
  NHANES 3-cycle ($n = 7{,}393$, zero outcome-fitted parameters).}
\label{tab:exercise}
\begin{tabular}{lccc}
\toprule
...
\caption{Organ-specific $|\Gamma|$ $\to$ cause-specific
  death AUC ($n \approx 52{,}545$, zero outcome-fitted parameters).}
...
zero outcome-fitted parameters) is comparable to fitted clinical
...
\caption{Pulse-pressure quartiles vs.\ vascular
  $|\Gamma|$ ($n = 72{,}886$, zero outcome-fitted parameters).}
\label{tab:pp-gamma}
\begin{tabular}{lcc}
\toprule
...
zero outcome-fitted parameters.

\begin{table}[h]
\centering
\caption{PTB-XL cardiac $\Gamma$ by diagnostic class
  (zero outcome-fitted parameters, $n = 484$).}
\label{tab:ptbxl}
\begin{tabular}{lcccc}
\toprule
...
\caption{Complete organ-specific $\Gamma$ validation summary
  (all zero outcome-fitted parameters).}
...
prediction, all with zero outcome-fitted parameters.
...
\begin{figure}[h]
  ...
  Relative risk $= 6.89\times$, zero outcome-fitted parameters.}
...
The $\Gamma$-framework, with zero outcome-fitted parameters,
...
\caption{Five-layer AUC progression
  ($n = 7{,}393$, NHANES 2013--2018).
  Layers~1--4 use zero outcome-fitted parameters;
...
\emph{zero outcome-fitted parameters}.
...
\bibitem{Paper4}
H.-Y.~Huang, ``Memory, Consciousness, and the Invariant
Topological Core as Impedance
