...
\subsection{Nocturnal thermal constraint}

Once on land, high-$K$ organisms face a second constraint.
During sleep, metabolic heat production drops to
$P_{\text{met}} \approx 82\,$W, sustaining only
$\Delta T = P_{\text{met}}/(h\,A) \approx 5.7\,^\circ$C
above ambient.
At $K{=}5$: the minimum viable ambient temperature is
$T_{\text{amb,min}} = T_c(5) - \Delta T \approx -1.2\,^\circ$C.
Temperate winter nights ($-10\,^\circ$C) push a sleeping
organism to $T_{\text{body}} \approx -4.3\,^\circ$C---well below
$T_c(5) = +4.5\,^\circ$C.

A cave at depth~$z$ acts as a thermal low-pass filter.
The penetration depth
$\delta = \sqrt{2\kappa/(\rho c_p\,\omega)}$ sets the
cutoff: at 5\,m, diurnal oscillation vanishes
($\delta_{\text{day}} \approx 0.13\,$m) and interior
temperature equals the annual mean
($\approx 12\,^\circ$C in temperate zones).
This yields
$T_{\text{body}}(\text{cave sleep}) \approx 17.7\,^\circ$C,
safely above $T_c$.
The constraint chain is:
high~$K$ $\to$ high~$T_c$ $\to$ nocturnal exposure risks
$T < T_c$ $\to$ thermal shelter required.
Low-$K$ organisms ($T_c(1) = -10.3\,^\circ$C) face no such
constraint and tolerate exposed sleeping sites.
This is a quantifiable physics prediction, not a narrative:
93~tests verify the full derivation from Arrhenius $\eta(T)$
through cave damping to survival margin.
