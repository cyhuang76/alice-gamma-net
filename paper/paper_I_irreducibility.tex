...
$\tau_{\text{conv}} \sim N^{-1.05}$ ($R^2 = 0.95$),
...
$N = 64$ network evolved for 200 ticks.
$\Aimp$ decays exponentially with time constant
$\tau \approx 20$ ticks, reaching $\Aimp < 10^{-3}$ by $t = 50$.
$\Acut$ is not constant but \emph{increases} monotonically
because spontaneous edge sprouting continually introduces new
cross-$K$ edges, each adding its dimensional cost.
This does not contradict the theorem, which states that
$\nabla_{\mathbf{Z}} \Acut \equiv 0$ at \emph{fixed topology};
the growth of $\Acut$ reflects topological exploration
(new edges), not impedance dynamics.
The theorem is validated: $\Aimp \to 0$ under C2 while
$\Acut$ accumulates as the topology explores.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/fig1_action_decomposition}
\caption{%
Action decomposition $A = \Aimp(t) + \Acut$.
Blue: $\Aimp \to 0$ (exponential decay).
Red dashed: $\Acut$ (irreducible, increasing with sprouting).
Inset: log-scale zoom of $\Aimp$ in the late-time regime.
$N = 64$, $\Delta K_{\max} = 2$.
}
\label{fig:action}
\end{figure}
...
The scaling $\tau_{\text{conv}} \sim N^{-1.05}$ is the most
surprising result.
