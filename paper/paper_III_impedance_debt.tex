...
As in Papers~I--II, all simulations use modules built from
the $\Gamma$-Net axioms.
When the sleep simulator reduces $D_Z$ exactly as the theory
predicts, it confirms that our code implements the equations
correctly---not that biological sleep obeys those equations.
Full experimental falsification of Predictions~1, 2, and~4
awaits comparative polysomnography across phyla.
We flag this circularity explicitly so that readers assess
our computational results as \emph{consistency checks},
...
\subsection{Scar tissue as permanent impedance debt}
\label{sec:scar-debt}

The wound-suturing process described in
Paper~II~\cite{PaperII} has a direct debt interpretation.
A fresh wound creates an acute, localised impedance debt:
\begin{equation}
  D_{Z,\text{wound}} = \int_0^{T_{\text{repair}}}
    \Gamma_{\text{wound}}^2(t)\,P_{\text{in}}(t)\,dt\,,
  \label{eq:wound-debt}
\end{equation}
where $\Gamma_{\text{wound}}^2(0)\approx 1$ at the moment
of injury and $P_{\text{in}}$ is the local metabolic
power demand (inflammation, angiogenesis, collagen
synthesis).

The biological repair process---haemostasis, inflammation,
proliferation, remodelling---discharges this debt by
progressively reducing $|\Gamma_{\text{wound}}|$.
However, scar tissue never achieves $\Gamma=0$:
the residual mismatch
$\Gamma_{\text{scar}}^2>0$ constitutes a
\emph{permanent, non-dischargeable impedance debt}
analogous to the irreducible topological cost
$A_{\text{cut}}$ from Paper~I.

This framework explains several clinical observations:
\begin{itemize}
  \item \textbf{Scar tenderness}: persistent $\Gamma^2>0$
    generates a continuous low-level nociceptive signal.
  \item \textbf{Scar contracture}: the system continues
    to remodel collagen fibre orientation for years,
    attempting to minimise $\Gamma^2$; the mechanical
    tension is a side-effect of this ongoing optimisation.
  \item \textbf{Keloid formation}: pathological
    over-deposition of collagen occurs when the repair
    loop overshoots, producing $Z_{\text{scar}}>Z_{\text{skin}}$
    and a new mismatch of opposite sign.
  \item \textbf{Cumulative surgical debt}: each surgical
    incision adds a permanent $\Gamma_{\text{scar}}^2$
    term to the body's total impedance debt budget.
\end{itemize}

\begin{remark}[Scar debt and the bathtub curve]
An individual who accumulates many scars (surgical,
traumatic) shifts their baseline $\Sigma\Gamma^2$
upward, effectively raising the floor of the lifecycle
bathtub curve (Paper~IV).  This provides a mechanistic
link between cumulative tissue trauma and accelerated
aging.
\end{remark}
...
experiment scripts and the 3\,274-test verification suite
