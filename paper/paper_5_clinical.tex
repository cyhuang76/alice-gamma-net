...
\usepackage{amsmath,amssymb,amsthm,bm,graphicx,booktabs,siunitx,hyperref}
...
interfaces across 14~orders of magnitude.

\begin{table*}[t]
\centering
\caption{Interface Scale Table --- the same
$\Gamma = (Z_2 - Z_1)/(Z_2 + Z_1)$ applies at every scale.}
\label{tab:scale}
\begin{tabular}{lllll}
\toprule
System & Scale & Effort & Flow & $Z$ definition \\
\midrule
Microtubule ion channel & 25~nm & Electric potential & Ion current & $V/I$ \\
Ion channel (Hodgkin--Huxley) & 10~nm & Membrane voltage & Ion current & $V/I$ \\
Synapse & 1~\textmu m & Post-synaptic potential & Synaptic current & $V/I$ \\
Dendritic cable (Rall) & 100~\textmu m & Membrane voltage & Axial current & $R_m / R_a$ per $\lambda$ \\
Axon (myelinated) & 1~mm & Voltage & Current & Characteristic impedance \\
Arteriole (Poiseuille) & 100~\textmu m & Pressure & Flow rate & $8\mu L / \pi r^4$ \\
Organ parenchyma & 10~cm & Metabolite conc. & Clearance flux & $[\text{met}]/J$ \\
Whole-body vascular & 1~m & Aortic pressure & Cardiac output & $P/Q$ \\
Ultrasound tissue boundary & 1~mm & Acoustic pressure & Particle velocity & $\rho c$ \\
Optical fiber splice & 100~\textmu m & Electric field & Magnetic field & $n \cdot Z_0$ \\
Power-grid transformer & 10~m & Voltage & Current & $V/I$ \\
Earth--ionosphere waveguide & $10^6$~m & E-field & H-field & $\sqrt{\mu/\varepsilon}$ \\
Submarine hull (acoustic) & 10~m & Pressure & Velocity & $\rho c$ \\
\bottomrule
\end{tabular}
\end{table*}
