...
\subsection{Tissue necrosis as thermodynamic substrate
  collapse}
\label{sec:necrosis}

The recovery analysis above
(Sec.~\ref{sec:recovery}) assumes that the
physical substrate---the material medium carrying
impedance---remains intact.
We now derive the condition under which the
substrate itself is destroyed: \emph{tissue necrosis}.

The impedance debt $D_Z = \int \Gamma^2\,P_{\text{in}}\,dt$
is, by construction, the cumulative reflected energy
that the tissue must either absorb or re-radiate.
The repair machinery (C2: $\Delta Z = -\eta\,\Gamma\,
x_{\text{in}}\,x_{\text{out}}$) converts a fraction
of this energy into structural adaptation.
When $D_Z$ accumulates faster than C2 can process:
\begin{equation}
  \frac{dD_Z}{dt}
  \;>\;
  \eta_{\max}\;|\Gamma|\;|x_{\text{in}}\,x_{\text{out}}|_{\max}
  \;\equiv\; \dot{D}_{\text{repair}}^{\max}\,,
  \label{eq:necrosis-condition}
\end{equation}
the excess energy has no impedance-remodeling
pathway and is dissipated as heat and
reactive biochemistry (free radicals,
proteolytic enzymes, membrane disruption).
...
and remodeling ($\partial Z_k/\partial t = 0$
regardless of input).
...
\noindent
A necrotic channel is a \textbf{topological black hole}:
it reflects all incident energy, emits toxic degradation
products (high-entropy noise) into the coupling matrix
$C_{kj}$, and cannot be repaired by any endogenous C2
process.
Unlike a high-$\Gamma$ pathological attractor
(Remark~\ref{rem:disease-phase-transition}),
which in principle can be reversed by sufficient
intervention, a necrotic channel has undergone a
\emph{thermodynamic phase transition} from
``mismatched but alive'' to
``structurally disintegrated.''

The coupling drag on the remaining network is
catastrophic.
In Eq.~(\ref{eq:coupled-G2}), a necrotic channel
contributes a constant $C_{kj} \times 1$ to every
coupled neighbour, plus toxic noise that degrades
neighbouring $\eta_j$.
Left unaddressed, the necrotic source
drives the global repair capacity $\eta(t)$ toward
zero---the definition of \emph{sepsis}:
systemic exhaustion of the impedance-repair budget
by a single overwhelming source of reflected energy
and entropic waste.

\subsubsection{Surgical intervention as topological
  truncation}

When the necrotic channel satisfies
Eq.~(\ref{eq:necrosis-condition}) irreversibly and its
coupling drag threatens systemic collapse, the only
viable intervention is \textbf{topological truncation}:
physically removing the necrotic node from the network.
...
\noindent
The physics of surgical truncation is:
\begin{enumerate}
  \item \textbf{Remove the noise source.}
    The necrotic channel's toxic emissions
    ($C_{kj} \times 1$ coupling +
    entropic degradation of $\eta_j$) are
    eliminated from the ODE system.
  \item \textbf{Create a clean boundary.}
    A surgically created $\Gamma = 1$ boundary
    (amputation stump, surgical margin) reflects
    $100\%$ of incident energy but produces
    \emph{no toxic noise}---it is an
    honest open circuit, not a necrotic one.
  \item \textbf{Release hijacked bandwidth.}
    The massive nociceptive crosstalk
    (Paper~4, Sec.~\ref{sec:gamma-pain-index})
    generated by the necrotic standing wave is
    terminated; the PFC's consciousness bandwidth
    is freed.
...
(Paper~4, phantom\_limb engine;
Sec.~\ref{sec:gamma-pain-index}):
...
Post-bifurcation & Pharmacotherapy + & Push $\Gamma_k^2$ \\
\quad (single) & \quad psychotherapy & below saddle-node \\
Post-bifurcation & Simultaneous & Multi-channel \\
\quad (multi) & \quad multi-modal & $\Gamma_k^2$ reset \\
Necrotic & Topological truncation &
  Remove node; clean \\
 & \quad (surgery) & $\Gamma{=}1$ boundary \\
\bottomrule
\end{tabular}
\end{center}
...
``selfish brain'' hypothesis~\cite{Peters2011},
which in $\Gamma$-Net topology is strictly an
\emph{asymmetric energy sink}: the brain occupies a
privileged topological hub whose low effective
impedance draws disproportionate power flux by
...
destructive.
The dormant-to-explosive SNR transition
(Paper~4, Eq.~ptsd-snr) applies identically:
quiet periods lower $P_{\text{noise}}$,
making even weak cravings breach
$\theta_{\text{recall}}$.
...
``self-control''---in control-theoretic terms,
a \emph{top-down prefrontal forcing function}
that attempts to override subcortical drive
signals---corresponds, in the $\Gamma$-language,
to the prefrontal repair rate $\eta_{\text{pfc}}(t)$.
...
% ============================================================
\subsection{Type~2 diabetes as global impedance drift:
  from receptor mismatch to six-channel deadlock}
\label{sec:t2d-derivation}

The preceding subsections established the motivational
cascade (Sec.~\ref{sec:obesity}) and the immune--metabolic
trap (Sec.~\ref{sec:imm-met-loop}).
We now derive the complete physics of type~2 diabetes
mellitus (T2DM) from the three constraints alone,
as the first non-neural disease model carried to
the same depth as the PTSD three-layer mechanism
(Paper~4, Sec.~7.10).

\subsubsection{Insulin signalling as impedance matching}
...
peripheral cells (muscle, liver, adipose)
present insulin receptors (the ``load'')
at impedance $Z_R$.
Under healthy matching,
$\Gamma_{\text{ins}} = (Z_R - Z_\beta)/(Z_R + Z_\beta) \approx 0$
and the transmission coefficient
$T_{\text{ins}} = 1 - \Gamma_{\text{ins}}^2 \approx 1$:
nearly all insulin signal is absorbed by the
receptor, glucose is cleared, and the loop
is quiescent.

Define the \emph{receptor impedance} in terms of
receptor density $N_R$ and binding affinity $K_d$:
\begin{equation}
  Z_R = \frac{Z_0}{K_d \cdot N_R}\,,
  \label{eq:z-receptor}
\end{equation}
where $Z_0$ is a tissue-type normalisation constant
(Table~\ref{tab:twenty-nine-tissues}, row~9).
Insulin resistance is \emph{not} a binary switch
but a continuous drift: either $N_R \downarrow$
(receptor downregulation) or $K_d \uparrow$
(binding affinity loss) raises $Z_R$,
increasing $\Gamma_{\text{ins}}^2$ monotonically.

\subsubsection{C2 remodeling of receptor impedance:
  the global baseline drift}
\label{sec:receptor-remodeling}

Unlike neural channels (where $Z$ changes along
specific axonal pathways), the insulin--receptor
system operates as a \emph{chemical bath}:
insulin is broadcast systemically, and every
cell's $Z_R$ is exposed to the same hormonal milieu.
A chronic stressor (elevated cortisol via the HPA
axis, Eq.~\ref{eq:selfish-brain}) acts as a
...
\begin{equation}
  ...
  + \underbrace{\alpha_c\,\Gamma_{\text{cortisol}}^2
    \,Z_R}_{\text{cortisol-driven upregulation}}\,,
  \label{eq:dZR-dt}
\end{equation}
where $\eta_R$ is the receptor repair rate
(analogous to neural $\eta$),
$x_{\text{ins}}$ and $x_{\text{glucose}}$ are
the insulin and glucose signals at the receptor,
and $\alpha_c$ is the cortisol coupling coefficient.
The first term is standard C2:
it drives $Z_R$ toward $Z_\beta$ (healthy matching).
The second term is the endocrine-specific physics:
chronically elevated cortisol
($\Gamma_{\text{cortisol}}^2 \gg 0$) acts as a
\emph{counter-gradient} that pushes $Z_R$ upward
faster than $\eta_R$ can pull it down.

This is the fundamental difference between
neural pathology and metabolic pathology.
In the neural case, C2 scars form on
\emph{individual pathways} (point-to-point
high-$\Gamma$ channels).
In the metabolic case, C2 remodeling acts on
\emph{all receptors globally}---a low-frequency,
whole-body impedance drift that is physically
equivalent to changing the dielectric constant of
the entire transmission-line medium.

\subsubsection{The insulin resistance lock:
  bifurcation analysis}
\label{sec:ir-lock}

Setting $dZ_R/dt = 0$ in Eq.~(\ref{eq:dZR-dt})
and solving for the equilibrium $\Gamma_{\text{ins}}$:
\begin{equation}
  \Gamma_{\text{ins}}^*
  = \frac{\alpha_c\,\Gamma_{\text{cortisol}}^2\,Z_R^*}
         {\eta_R\,x_{\text{ins}}\,x_{\text{glucose}}}\,.
  \label{eq:ir-equilibrium}
\end{equation}
When $\Gamma_{\text{cortisol}}^2$ is small
(low stress), $\Gamma_{\text{ins}}^* \approx 0$
and the servo holds.
As stress chronically elevates cortisol, the
equilibrium $\Gamma_{\text{ins}}^*$ drifts toward 1.

The critical regime is the \emph{positive feedback
threshold}.
...
But elevated insulin itself drives receptor
downregulation ($N_R \downarrow$), further raising
$Z_R$:
\begin{equation}
  \frac{dN_R}{dt}
...
$\sigma_R$ is the receptor synthesis rate.
Substituting into Eq.~(\ref{eq:z-receptor}),
the feedback loop closes:
\[
  \Gamma_{\text{ins}}^2\!\uparrow
  \;\xrightarrow{\text{hyperglycaemia}}
  \text{insulin}\!\uparrow
  \;\xrightarrow{\kappa}
  N_R\!\downarrow
  \;\xrightarrow{Z_R\uparrow}
  \Gamma_{\text{ins}}^2\!\uparrow
  \;\to\;\cdots
\]

This is structurally identical to the immune--metabolic
trap of Eqs.~(\ref{eq:imm-loop})--(\ref{eq:met-loop}),
but occurring \emph{within} the endocrine channel.
The bifurcation condition is:
\begin{equation}
  \kappa\,[\text{insulin}]\,\frac{\partial Z_R}
    {\partial N_R}
  > \eta_R
  \quad
  \Longleftrightarrow
...
attractor---\emph{insulin resistance is structurally
irreversible under the current $\eta_R$}, precisely
analogous to the PTSD scar being locked in
$\mathcal{S}_{\text{eff}}$.
...
Eq.~(\ref{eq:dZR-dt}) by lowering the product
$x_{\text{ins}} \cdot x_{\text{glucose}}$.
If the cortisol term
$\alpha_c\,\Gamma_{\text{cortisol}}^2$ remains
elevated (chronic stress persists), the net
$dZ_R/dt$ may remain positive \emph{despite caloric
deficit}---the receptor impedance continues to
drift upward even as the patient loses weight.
This resolves the clinical paradox of patients
who achieve significant weight loss yet remain
insulin-resistant: the \emph{endocrine} channel
($Z_R$ drift) was never addressed because the
intervention targeted only the \emph{metabolic}
channel ($\Gamma_{\text{met}}^2$).
\end{remark}

\subsubsection{The six-channel T2DM cascade}
\label{sec:t2d-six-channel}

Once $\Gamma_{\text{ins}}^2$ crosses the bifurcation
threshold of Eq.~(\ref{eq:ir-bifurcation}),
the coupled dynamics of
Eq.~(\ref{eq:coupled-G2}) propagate the mismatch
through the full coupling matrix $C_{kj}$.
We trace the complete cascade through six channels:

\begin{enumerate}
  \item \textbf{Neural $\to$ Endocrine}
    ($C_{\text{end,neu}}$):
    Chronic stress raises
    $\Gamma_{\text{cortisol}}^2$ via the HPA axis.
    This is the \emph{initiating event}---the
    ``match that lights the fire.''
    The cortisol signal acts as a global
    $x_{\text{in}}$ that drives receptor
    $Z_R$ upward (Eq.~\ref{eq:dZR-dt}).

  \item \textbf{Endocrine $\to$ Metabolic}
    ($C_{\text{met,end}}$):
    As $\Gamma_{\text{ins}}^2 \to 1$,
    glucose clearance collapses
    ($T_{\text{ins}} \to 0$).
    Hyperglycaemia and
    hyperinsulinaemia both rise;
    hepatic lipogenesis accelerates
    (the liver converts excess glucose
    to fat when the metabolic channel
    cannot absorb it).
    $\Gamma_{\text{met}}^2$ rises.

  \item \textbf{Metabolic $\to$ Immune}
    ($C_{\text{imm,met}}$):
    Adipocyte hypertrophy from lipogenesis
    triggers macrophage infiltration and
    pro-inflammatory cytokine release
    (IL-6, TNF-$\alpha$).
    $\Gamma_{\text{imm}}^2$ rises---the
    inflammation--insulin trap of
    Sec.~\ref{sec:imm-met-loop} activates.

  \item \textbf{Immune $\to$ Vascular}
    ($C_{\text{vas,imm}}$):
    Chronic inflammation damages endothelial
    function.
    The vascular channel's $\Gamma^2$ rises:
    arterial compliance drops per the
    Moens-Korteweg equation
    $c = \sqrt{Eh/2\rho r}$,
    where increased wall thickness $h$
    (fibrosis) and decreased radius $r$
    (lumen narrowing) both elevate
    pulse-wave velocity---the vascular
    analogue of rising $Z$.
    This is the physics of diabetic
    macrovascular disease.

  \item \textbf{Vascular $\to$ Renal}
    ($C_{\text{ren,vas}}$):
    Elevated vascular $\Gamma^2$ reduces
    glomerular perfusion.
    The renal channel compensates by
    activating RAAS
    (renin--angiotensin--aldosterone),
    further raising systemic vascular $Z$---a
    second positive-feedback loop.
    Proteinuria and declining GFR
    are the impedance signatures of
    diabetic nephropathy.

  \item \textbf{Renal $\to$ Neural}
    ($C_{\text{neu,ren}}$):
    Uraemic toxins from declining renal function
    impair neural repair rate $\eta_n$,
    further depressing the brain's ability
    to modulate the HPA axis---closing the
    six-channel loop back to channel~1.
\end{enumerate}
...
above their respective bifurcation thresholds
(regime~3 of Sec.~\ref{sec:recovery}),
...
\end{equation}
This is Eq.~(\ref{eq:irreversible}) instantiated
for the T2DM hexagon.
The clinical implication is immediate: treating
only blood sugar (metformin targeting
$\Gamma_{\text{met}}^2$) without simultaneously
addressing stress ($\Gamma_{\text{cortisol}}^2$),
inflammation ($\Gamma_{\text{imm}}^2$), and
vascular damage ($\Gamma_{\text{vas}}^2$)
leaves the locked channels to re-drive
$\Gamma_{\text{ins}}^2$ from the other side
of the hexagon.
...
channel $k$ receives drive from its neighbours
that exceeds its own repair capacity.
Unlocking channel $k$ alone reduces
$\Gamma_k^2$ transiently, but the coupling
$C_{kj}\,\Gamma_j^2$ from the remaining locked
channels $j \in \mathcal{L} \setminus \{k\}$
re-elevates $\Gamma_k^2$ above threshold within
$\tau_{\text{relapse}} \sim C_{kj}/\eta_k$ ticks.
Only simultaneous reduction of all
$\Gamma_j^2$ for $j \in \mathcal{L}$ breaks
the mutual sustenance condition.
$\square$
\end{theorem}

This theorem explains the well-documented clinical
observation that T2DM management improves
dramatically under ``lifestyle medicine''
protocols that combine dietary change (metabolic),
exercise (vascular + metabolic), stress reduction
(neural + endocrine), and anti-inflammatory
agents (immune)---each targeting a different
vertex of the hexagon.
It also predicts that GLP-1 receptor agonists
(which simultaneously reduce appetite, lower
$\Gamma_{\text{met}}^2$, and exhibit
anti-inflammatory properties) should outperform
agents that target only one channel---a prediction
consistent with the clinical superiority of
semaglutide over metformin monotherapy.

\begin{remark}[$\beta$-cell exhaustion as
impedance substrate collapse]
\label{rem:beta-exhaustion}
In advanced T2DM, the compensatory
hyperinsulinaemia of stage~2 eventually
exhausts $\beta$-cell secretory capacity:
$Z_\beta \to \infty$ (cell death).
This is the metabolic analogue of tissue
necrosis (Paper~5, Sec.~\ref{sec:necrosis}):
the substrate that \emph{generates} the
source impedance is destroyed, and no
amount of C2 remodeling can restore a
channel whose source no longer exists.
At this stage, T2DM functionally
converges with T1DM---not because the
aetiology is the same, but because the
\emph{topological endpoint} is identical:
$Z_\beta = \infty$, $\Gamma_{\text{ins}} = 1$,
$T_{\text{ins}} = 0$.
Exogenous insulin injection is the only
remaining intervention: it replaces the
destroyed source by providing an external
$Z_\beta$ to the network.
\end{remark}
...
\begin{enumerate}
  ...
  \item \textbf{T2DM multi-channel treatment superiority.}
    ...
    This follows from Theorem~\ref{thm:t2d-treatment}:
...
deadlock (Sec.~\ref{sec:t2d-derivation}),
including the receptor impedance drift equation
(Eq.~\ref{eq:dZR-dt}), the bifurcation condition
(Eq.~\ref{eq:ir-bifurcation}), and the
...
(water-hammer physics in compliant vessels)---are
at present formulated at the level of coupled
ODE scaffolds
(Eqs.~\ref{eq:imm-loop}--\ref{eq:met-loop},
Eq.~\ref{eq:coupled-G2}) but have not yet been
carried to bifurcation-condition depth.

We regard this as a clear and honest boundary:
\emph{Phase~I} (v3.8.0) has completed
the first-principles derivation for the
neural--endocrine--metabolic axis
(Papers~3, 4, and this paper's
Secs.~\ref{sec:obesity}--\ref{sec:t2d-derivation}).
\emph{Phase~II} will extend the same derivation
methodology to the immune field-allocation problem
and the cardiovascular fluid-reflection problem.
The coupled ODE framework
(Eq.~\ref{eq:coupled-G2}) and the
irreversibility conditions
(Eq.~\ref{eq:irreversible}) already provide the
skeleton; what remains is the
channel-specific bifurcation analysis at the
same depth as Eq.~(\ref{eq:ir-bifurcation}).
...
We have established seven results.
...
\textbf{Sixth}, when impedance debt accumulates beyond
the substrate's thermodynamic repair capacity
($dD_Z/dt > \dot{D}_{\text{repair}}^{\max}$),
the tissue undergoes irreversible substrate collapse
(necrosis): $\Gamma = 1$ permanently, with toxic
coupling drag that can exhaust the global repair budget
(sepsis).
The only viable intervention is topological
truncation---surgical removal of the necrotic node
and creation of a clean $\Gamma = 1$ boundary---which
connects directly to phantom limb pain (Paper~4)
when the motor cortex refuses to acknowledge the
new boundary.

\textbf{Seventh}, the complete physics of type~2
diabetes mellitus has been derived from C2 alone
(Sec.~\ref{sec:t2d-derivation}): insulin resistance
is a global impedance drift
($dZ_R/dt$, Eq.~\ref{eq:dZR-dt}) driven by
chronic cortisol counter-gradient;
the bifurcation condition
(Eq.~\ref{eq:ir-bifurcation}) locks the receptor
in a high-$\Gamma_{\text{ins}}^2$ attractor;
and the six-channel hexagonal cascade
(Eq.~\ref{eq:t2d-hexagon}) propagates the mismatch
through endocrine, metabolic, immune, vascular,
and renal channels back to neural---forming a
closed deadlock that single-channel therapy
cannot break
(Theorem~\ref{thm:t2d-treatment}).
This is the first non-neural disease model
carried to the same derivation depth as the
PTSD three-layer mechanism (Paper~4),
demonstrating that the $\Gamma$-framework
extends beyond neuroscience to whole-body
metabolic pathology.
