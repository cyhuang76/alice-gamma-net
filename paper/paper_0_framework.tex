...
\paragraph{Nocturnal thermal constraint.}
During sleep, metabolic heat production drops to
$P_{\text{met}} \approx 82\,$W, yielding a thermal excess
$\Delta T = P_{\text{met}}/(h\,A) \approx 5.7\,^\circ$C
above ambient.
For $K{=}5$: the minimum viable ambient temperature is
$T_{\text{amb,min}} = T_c(5) - \Delta T \approx -1.2\,^\circ$C.
Temperate winter nights ($-10\,^\circ$C) push
$T_{\text{body}}$ to $-4.3\,^\circ$C---well below $T_c$.
Thermal shelter becomes a \emph{physical necessity}.
A cave at depth~$z$ damps surface oscillations by
$\exp(-z/\delta)$, where the thermal penetration depth
$\delta = \sqrt{2\kappa/(\rho c_p\,\omega)}$.
For limestone at 5\,m: diurnal oscillation is eliminated
($\delta_{\text{day}} \approx 0.13\,$m), and
cave temperature equals the annual mean
($\approx 12\,^\circ$C in temperate zones).
This yields $T_{\text{body}}(\text{cave sleep}) \approx
17.7\,^\circ$C, safely above $T_c(5) = 4.5\,^\circ$C.
Low-$K$ organisms face no such constraint:
$T_c(1) = -10.3\,^\circ$C permits exposed survival.
The constraint chain is:
high~$K$ $\to$ high~$T_c$ $\to$ endothermy $\to$
sleep reduces $P_{\text{met}}$ $\to$ nocturnal exposure
risks $T < T_c$ $\to$ thermal shelter required.
