...
%--------------------------------------------------------------------
\title{Paper~2: Dual Impedance Networks:\\
...
%--------------------------------------------------------------------
\begin{abstract}
Paper~1 of this series established that the Minimum Reflection
...
randomised parameter sweeps ($N=50$), and five clinical severity
...
Paper~1~\cite{Paper1} established that the Minimum Reflection
Principle~(MRP),
...
Paper~3 of this series~\cite{Paper3}
extends the framework to daily dynamics
...
\end{tabular}
\end{center}
The table reveals why MRP transfers intact from
electrical to hemodynamic media.
Because \(\Gamma = (Z_{\text{load}} - Z_{\text{source}})
/ (Z_{\text{load}} + Z_{\text{source}})\) depends only
on the \emph{ratio} of effort to flow, the variational
principle \(\delta\!\int\!\Gamma^2\,dt = 0\) is
medium-independent:
the physical dimension of \(Z\) changes
(\(\Omega\) for wires, Pa\(\cdot\)s/m\(^3\) for arteries,
N\(\cdot\)s/m for tendons), but the mathematical
structure---and therefore all derived constraints
C1, C2, C3---is identical.
...
%====================================================================
\section{Vascular Impedance Remodeling}\label{sec:hebbian}
%====================================================================

Neural MRP descent uses the impedance remodeling rule~(C2):
$\Delta Z = -\eta \cdot \Gamma \cdot x_{\text{in}} \cdot
x_{\text{out}}$.
...
\begin{equation}\label{eq:hebb_v}
  \Delta \Zv = -\eta_{\mathrm{v}} \cdot \Gv
              \cdot \underbrace{\tau_{\text{wall}}}_{\text{shear stress}\,\sim\, x_{\text{in}}}
              \cdot \underbrace{\rho}_{\text{growth factor}\,\sim\, x_{\text{out}}}\,,
\end{equation}
...
\emph{Key difference from neural C2 remodeling:}
...
$H$ is the organ's \emph{global power-transfer efficiency}:
the fraction of incident power that traverses both
impedance networks and reaches the tissue.
Clinically, $H \approx 1$ corresponds to a healthy organ;
$H \to 0$ to organ failure.
Both networks must be well matched for the organ
to function normally.

\paragraph{The vascular network as thermal bus.}
Paper~0, Corollary~1~\cite{Paper0} shows that every
neural interface dissipates power $\Gn^2 P_{\mathrm{in}}$
as heat, and Paper~1~\cite{Paper1} shows that volumetric
heat density scales with~$K$.  Convective blood flow is
the \emph{only} mechanism fast enough to remove this
heat from deep tissue (conduction alone yields a thermal
time constant $\tau_{\mathrm{cond}} \sim L^2/\alpha
\approx 10^3$\,s for $L\sim$cm, far too slow for neural
time scales).  The vascular network is therefore not
merely a nutrient bus---it is the \emph{primary thermal
bus}:
\begin{equation}
  P_{\mathrm{removed}} = \rho\, c_p\, Q\,
  (T_{\mathrm{out}} - T_{\mathrm{in}})
  = (1 - \Gv^2)\,\rho\, c_p\, Q_0\, \Delta T\,,
  \label{eq:thermal-bus}
\end{equation}
where $Q = (1-\Gv^2)Q_0$ is the actual flow (reduced by
vascular mismatch), $\rho c_p$ is the volumetric heat
capacity of blood, and $\Delta T$ is the arteriovenous
temperature difference.  Vascular disease
($\Gv^2\!\uparrow$) therefore simultaneously starves
the tissue of nutrients \emph{and} impairs its thermal
regulation---a dual failure that accelerates neural
impedance drift.
...
\begin{enumerate}
  ...
  \item \emph{Material starvation}: reduced $\rho$ means less
    glucose/O$_2$ for neural repair, so neural C2
...
\begin{enumerate}
  ...
  \item \textbf{Endothelial dysfunction} $\to$ impaired
    impedance remodeling ($\eta_v \to 0$).
...
\begin{enumerate}
  ...
  \item[\textbf{V7.}] \textbf{Vascular impedance remodeling.}
...
explained by vascular impedance remodeling: early
$\Gv$ reduction drives vessel wall adaptation
($\Delta Z_v = -\eta_v \Gv \cdot \tau_{\text{in}} \cdot \rho_{\text{out}}$)
...
\paragraph{Neural C2 self-repair.}
During this analysis we discovered that the bare simulation lacks a
neural-side analogue of the vascular impedance remodeling
...
compliant vascular bed), confirming that the impedance
...
\begin{remark}[Analytic stability guarantee]
\label{rem:stability-analytic}
The 100\% numerical pass rate has an analytic
underpinning.
The reflection action
$\mathcal{A}[\Gamma] = \int\!\sum_i \Gamma_i^2\,dt$
serves as a Lyapunov function for the coupled system:
C2 ensures $d\mathcal{A}/dt \le 0$ (Paper~1,
Theorem~1), so $\Sigma\Gamma^2$ is non-increasing
and bounded below by~$A_{\text{cut}} > 0$.
Stability under the dual-network time-scale mismatch
($\eta_n \approx 0.008 \gg \eta_v \approx 0.001$)
follows from a singular-perturbation argument:
defining $\epsilon = \eta_v / \eta_n \ll 1$,
the fast (neural) subsystem relaxes to its
quasi-static manifold on the time scale
$1/\eta_n$ while the slow (vascular) subsystem
provides a drifting boundary condition.
Standard Tikhonov--Fenichel theory guarantees that
if the fast subsystem is asymptotically stable on
its slow manifold---which it is, because $\mathcal{A}$
is a Lyapunov function---then the full system is
asymptotically stable for sufficiently small
$\epsilon$.
A complete formal proof, including explicit bounds
on the basin of attraction under pathological
perturbations, remains an open problem and is
listed in \texttt{KNOWN\_LIMITATIONS.md}.
\end{remark}
...
Paper~1 treated only the neural $\Gamma$.
...
\subsection{From dual to quintuple: five irreducible subsystems}
\label{sec:quintuple}

The dual-network result generalises.
C2~remodelling is a \emph{dissipative} process:
every update $\Delta Z = -\eta\,\Gamma\,x_{\text{in}}\,x_{\text{out}}$
consumes material substrate---glucose, O$_2$,
amino acids, iron, folate---that cannot be synthesised
\emph{de novo} by either network.
These substrates originate outside the organism at
an environmental impedance $Z_{\text{env}}$ (chemical
bond energy, macromolecular structure) severely
mismatched to intracellular impedance $Z_{\text{bio}}$:
$\Gamma_{\text{raw}} = (Z_{\text{bio}} - Z_{\text{env}})
/ (Z_{\text{bio}} + Z_{\text{env}}) \approx 1$.
Without a \emph{multi-stage impedance transformer}
that progressively lowers $\Gamma$ from ${\approx}\,1$
to ${\approx}\,0$, no substrate enters the blood.

The gastrointestinal tract is exactly this transformer:
oral cavity (mechanical fragmentation, amylase),
stomach (acid denaturation, pepsin),
small intestine (bile emulsification, pancreatic hydrolysis),
and hepatic portal clearance each reduce the
environment-to-blood impedance mismatch by one stage---
functionally identical to the relay hierarchy of
Paper~1, Sec.~5.
Similarly, the skin boundary defines the thermal
resistance $R_{\text{th}}$ that determines whether the
organism can shed C2~waste heat
(Paper~1, Eq.~\ref*{eq:Rth-K});
removing it leaves $R_{\text{th}}$ undefined and
thermal run-away inevitable.

\paragraph{Scope distinction.}
The dual-network metric
$H = \Tn\,\Tv$ (Eq.~\ref{eq:dual_health})
operates \emph{within} each organ: the gut, for instance,
has its own $\Gn$ (enteric neural matching) and
$\Gv$ (mesenteric vascular matching),
both validated in Sec.~\ref{sec:cascade}.
The five-subsystem chain below operates at a
\emph{different hierarchical level}---it describes the
serial cascade of \emph{whole-body} prerequisites that
must all transmit for any organ-level $H$ to be defined
at all.
Physically, the five subsystems form a
\textbf{series transmission line}:
\[
  \underbrace{\text{Env}}_{\displaystyle Z_{\text{env}}}
  \!\xrightarrow{\;\Gamma_{\text{dig}}\;}
  \underbrace{\text{Blood}}_{\displaystyle Z_{\text{bus}}}
  \!\xrightarrow{\;\Gamma_{\text{bld}}\;}
  \underbrace{\text{Vasc}}_{\displaystyle Z_{\text{v}}}
  \!\xrightarrow{\;\Gamma_{\text{v}}\;}
  \underbrace{\text{Neural}}_{\displaystyle Z_{\text{n}}}
  \!\xrightarrow{\;\Gamma_{\text{n}}\;}
  \underbrace{\text{Skin}}_{\displaystyle R_{\text{th}}}
  \!\to\text{heat sink}\,.
\]
In a cascade of impedance stages the total
power transmission is the product of the individual
stage transmissions---exactly as in a chain of
microwave connectors or optical coatings.
The product form is therefore not an assumption of
independence; it is the physics of series junctions.

We identify \textbf{five mutually irreducible
subsystems}, each of which, if removed, drives
$\Gamma \to 1$ in finite time:

\begin{center}
\begin{tabular}{lll}
\hline
Subsystem & Irreducible role & Failure time-scale \\
\hline
Neural      & C2 executor ($\Delta Z$ computation)
            & Immediate \\
Vascular    & $\rho$-transport $+$ thermal bus
            & Minutes \\
Blood       & Transmission medium ($Z_{\text{bus}}$)
            & Minutes \\
Skin        & Thermal boundary ($R_{\text{th}}$)
            & Hours \\
Digestive   & Environment $\to$ blood impedance matching
            & Days--weeks \\
\hline
\end{tabular}
\end{center}
The descending time-scale reflects the body's
substrate reserves: glycogen $\sim$24\,h, lipid
$\sim$weeks, protein $\sim$months.
The whole-organism viability therefore requires
\begin{equation}
  H_{\text{org}} = \prod_{s=1}^{5}(1-\Gamma_s^2)\,,
  \label{eq:quintuple_health}
\end{equation}
where the product runs over the five \emph{serial}
stages, not over independent channels.
For any specific organ, the full health metric
nests two levels:
$H_{\text{full}} = H_{\text{org}} \cdot
 (1 - \Gamma_{\mathrm{n,local}}^2)(1 - \Gamma_{\mathrm{v,local}}^2)$,
recovering the intra-organ dual product of
Eq.~\eqref{eq:dual_health}
only when all upstream stages transmit
($H_{\text{org}} \approx 1$).
Any single $\Gamma_s \to 1$ collapses the product to zero,
formalising the clinical observation that
multi-system failure is universally lethal regardless
of which subsystem fails first.
...
The $\Gamma$-Pain Index $\Pi_{\Gamma}$ (Paper~4)
...
debt of the wound (Eq.~(\ref{eq:wound-debt}) in Paper~3).
...
Together with Paper~1, this establishes the MRP
as a universal physics of biological impedance networks:
\begin{itemize}
  \item Paper~1: Static architecture (relay nodes, fractal topology)
  \item Paper~2 (this work): Vascular network and dual-organ model
  \item Paper~3: Temporal dynamics (impedance debt, sleep, lifecycle, aging)
\end{itemize}
...
\bibitem{Paper0}
H.-Y.~Huang,
``Paper~0: The Gamma-Net Framework---Axioms, Constraints,
  and the Minimum Reflection Action,''
preprint (2026).
