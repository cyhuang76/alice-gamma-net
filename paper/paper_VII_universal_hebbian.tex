...
\subsection{Joint as Multi-Layer Impedance Transformer}
\label{sec:joint-transformer}

The cartilage $\eta \approx 0$ section above describes
a failure mode.  We now show that the \emph{healthy}
joint is a textbook example of impedance-graded
relay-node architecture (Paper~I, Corollary~2).

An articular joint couples bone ($Z_{\text{bone}} = 120$)
to synovial fluid ($Z_{\text{syn}} = 50$) through a
five-layer stack:
\[
  \text{Bone}(120) \;\to\; \text{Cartilage}(80)
  \;\to\; \text{Synovial}(50)
  \;\to\; \text{Cartilage}(80)
  \;\to\; \text{Bone}(120)\,.
\]
The cartilage impedance $Z_{\text{cart}} = 80 \approx
\sqrt{Z_{\text{bone}} \cdot Z_{\text{syn}}} = 77.5$
is the geometric mean---the exact condition for a
quarter-wave impedance transformer~\cite{Pozar2011}
that minimises reflection at each interface.
Each boundary carries $\Gamma^2 < 0.10$, and
the total transmission $T_{\text{total}} > 0.80$.
C1 ($\Gamma^2 + T = 1$) is satisfied at every interface.

\paragraph{Aging pain immunity.}
A natural corollary: if $Z$ drifts slowly with age
but C2 ($\eta > 0$) continuously re-matches the
impedance, $\Gamma^2$ remains $O(10^{-6})$---far below
the nociceptive threshold ($T > 0.7$, see Paper~IV).
Pain triggers only when $dZ/dt$ exceeds C2 capacity
(acute injury), or when $\eta \to 0$
(cartilage OA, see above).
This explains why normal aging increases tissue
impedance but \emph{does not produce chronic pain}---a
prediction verified numerically~\cite{ALICE_code}
(3\,274 tests, 27/27 orthopedic tests passed).

\paragraph{End-stage osteoarthritis.}
When cartilage is fully eroded, the joint becomes
bone-on-bone: $\Gamma_{\text{direct}} = (120-50)/(120+50)
= 0.41$, $T = 0.83 \to 0.59$.
The relay node is physically removed, increasing
$A_{\text{cut}}$ at the joint and producing the reflection
heat ($\Gamma^2 \cdot P_{\text{in}}$) that drives
chronic OA pain.
