# -*- coding: utf-8 -*-
"""Tests for ConsciousnessModule — Screen Brightness Model

Φ = screen_brightness = mean(T_i) from ConsciousnessScreen.
No weighted formula. The screen is either on or off.
"""
import time
import numpy as np
import pytest
...
class TestPhiCalculation:
    def test_high_brightness_high_phi(self):
        """Bright screen → high Φ (conscious)."""
        c = ConsciousnessModule()
        result = c.tick(screen_brightness=0.9)
        # EMA from 0.8 toward 0.9
        assert result["phi"] > CONSCIOUS_THRESHOLD

    def test_zero_brightness_drops_phi(self):
        """Φ converges to 0 when screen is off."""
        c = ConsciousnessModule()
        for _ in range(30):
            result = c.tick(screen_brightness=0.0)
        assert result["phi"] < SUBLIMINAL_THRESHOLD

    def test_dim_screen_lower_phi(self):
        """Dimmer screen → lower Φ."""
        c_bright = ConsciousnessModule()
        c_dim = ConsciousnessModule()
        for _ in range(20):
            r_bright = c_bright.tick(screen_brightness=0.9)
            r_dim = c_dim.tick(screen_brightness=0.3)
        assert r_bright["phi"] > r_dim["phi"]

    def test_pain_affects_screen_not_formula(self):
        """Pain level is recorded for meta-awareness but does not alter Φ.
        (Pain affects screen via overload in ConsciousnessScreen, not here.)"""
        c1 = ConsciousnessModule()
        c2 = ConsciousnessModule()
        for _ in range(10):
            r1 = c1.tick(screen_brightness=0.6, pain_level=0.0)
            r2 = c2.tick(screen_brightness=0.6, pain_level=0.9)
        # Same screen brightness → same Φ
        assert r1["phi"] == pytest.approx(r2["phi"], abs=0.01)
...
class TestConsciousnessStates:
    ...
    def test_conscious_state(self):
        c = ConsciousnessModule()
        for _ in range(30):
            result = c.tick(screen_brightness=0.55)
        assert c.is_conscious()
...
class TestMetaAwareness:
    def test_meta_aware_when_lucid(self):
        ...
        for _ in range(20):
            result = c.tick(screen_brightness=0.9,
                            attention_strength=0.9, arousal=0.9)
    ...
    def test_not_meta_aware_when_low(self):
        c = ConsciousnessModule()
        for _ in range(30):
            result = c.tick(screen_brightness=0.1,
                            attention_strength=0.1, arousal=0.1)
...
class TestComponentOutput:
    def test_components_in_result(self):
        c = ConsciousnessModule()
        result = c.tick(screen_brightness=0.6, attention_strength=0.5,
                        binding_quality=0.6, arousal=0.7,
                        sensory_gate=0.8, pain_level=0.3)
        comp = result["components"]
        assert "screen_brightness" in comp
        assert "attention" in comp
        assert "binding" in comp
        assert "arousal" in comp
        assert "pain_level" in comp
        assert comp["screen_brightness"] == pytest.approx(0.6)
