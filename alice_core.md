...
> **版本**：v1.5 | **建立日期**：2026-03-16 | **上次更新**：2026-03-25 | **更新規則**：僅在重大理論突破或確立新公理時更新
...
## 三之三、意識 × 記憶 × 認知的物理機制（P1 §5–§7, 2026-03-24）

### 意識三層結構
- **意識** = Television Inequality：$Z_{\text{arousal}} \cdot \sum G_i \cdot Q_{\text{PFC}} > \theta$（基線「醒著」）
- **注意力** = $\Gamma^2 \neq 0$ 觸發 gate → 選擇性處理
- **主觀體驗** = 意識 × 注意力；「我是我」= $S_{\text{eff}}$ 的注意力模式

### 杏仁核 = 意識的測量裝置
- 偵測 $\Gamma^2$（30–74 ms）→ 調控 $Z_{\text{arousal}}$、$G_i$、$\eta$
- PFC = 處理器（200–300 ms），杏仁核 = 測量儀（30–74 ms）
- 電視 vs 大腦的精確差異：**電視沒有測量裝置**

### 「我」的物理本質
- $\Gamma^2 = 0$：邊界消失 → 合一（涅槃）→ 無我
- $\Gamma^2 = 1$：邊界封閉 → 隔絕（死亡）→ 無我
- $0 < \Gamma^2 < 1$：部分邊界 → **「我」存在**
- Johnson–Nyquist 熱雜訊設定下界 → **「我」是熱雜訊的副產物**
- 杏仁核反 $\Gamma^2 \to 0$ 防禦：開悟需重寫進化 $S_{\text{eff}}$（數十年訓練）

### 記憶機制調和
- **海馬迴** = 情景記憶的 C3 relay（非記憶中心）→ H.M. 驗證
- **記憶** = Z-pattern 配置本身（不存在分開的「存儲位置」）→ engram cell = 入口鑰匙
- **LTP/LTD** = 同一 C2 方程的兩面（Z 移向 B = LTP for B, LTD for A）
- **小腦** = 平行低 K C2 引擎（80% 神經元 in 10% 體積）

### 認知功能（全部由 C2 + gate 機制推導）
- **做夢** = REM gate-leak 隨機綁定碎片記憶
- **注意力/ADHD** = gate $\tau_G$ 時間常數（ADHD = $\tau_G$ 過短）
- **情緒** = 杏仁核低 K 快速廣播 + gate override
- **創造力** = gate 放鬆 → 多通道遠距綁定
- **直覺** = $S_{\text{eff}}$ 直接輸出（跳過 PFC）
- **共情** = $S_{\text{eff}}$ 共振 → $\Gamma^2_{\text{empathy}} \neq 0$
- **心理治療** = 結構化的 guided C2 descent（共振 + 引導再暴露 + 行為處方）
- **第六感** = 杏仁核亞閾值偵測（30 ms）→ PFC 尚未意識到
- **遠距感應** = $S_{\text{eff}}$ 趨同 + 確認偏誤（腦對腦 SNR < 1 at >3 cm → 物理不可能）
- **自信心** = 願意在 PFC 確認前行動 → C2 可訓練（驗證 → LTP，否定 → LTD）
- **多巴胺** = $\eta$ 調控器（$\eta^* \propto -d\Gamma^2/dt$）→ 成癮 = $\eta$ 劫持
- **神經循環** = 清醒（C2+多巴胺）→ 代謝（ATP→腺苷）→ 關機 → 清除 → 重置
...
| Identity | 注意力模式 | $\{\alpha : \Gamma^2_\alpha \text{ triggers } G_\alpha\}$ | P1 |
| $\Gamma^2_{\text{empathy}}$ | 共情係數 | $|Z_{\text{other}} - Z_{\text{self}}^{S}|^2/|Z_{\text{other}} + Z_{\text{self}}^{S}|^2$ | P1 |
...
Layer 1 (P1) | C2 在有限網路 → A_cut>0 → 拓撲、大腦、意識、記憶、靈魂、注意力、共情、自信
