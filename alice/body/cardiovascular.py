...
# --- Blood viscosity (Quemada-inspired nonlinear model) ---
VISCOSITY_NORMAL = 1.0               # Normalized at full hydration
# Polynomial coefficients: η = η₀ × (1 + a·d + b·d³)
# Physical basis: Quemada equation for hematocrit-dependent viscosity
# Low dehydration (d<0.3): nearly linear (≈ 1 + 0.6d)
# Severe dehydration (d>0.7): cubic term dominates → rapid viscosity surge
VISCOSITY_COEFF_LINEAR = 0.6         # Linear coefficient a
VISCOSITY_COEFF_CUBIC = 2.5          # Cubic coefficient b (drives nonlinearity)
...
class CardiovascularSystem:
    ...
    def _update_viscosity(self) -> None:
        ...
        Quemada-inspired model: η = η₀ × (1 + a·d + b·d³)
        where d = dehydration deficit [0, 1].

        Physical basis:
        - Hematocrit rises with dehydration (blood concentrates)
        - RBC aggregation at high Hct causes viscosity to surge nonlinearly
        - Cubic polynomial avoids Quemada singularity (1/(1-k·Hc/Hcm)²)
          while preserving the essential nonlinear behavior
        - Smooth monotonic increase: no oscillation, continuous at all d
        """
        deficit = max(0.0, 1.0 - self._blood_volume / BLOOD_VOLUME_SETPOINT)
        viscosity = VISCOSITY_NORMAL * (
            1.0
            + VISCOSITY_COEFF_LINEAR * deficit
            + VISCOSITY_COEFF_CUBIC * deficit ** 3
        )
    ...
    def _update_stroke_volume(self) -> None:
        """
        Stroke volume = f(blood_volume, contractility, viscosity).

        Less blood → less venous return → less filling → lower SV.
        Better contractility → stronger ejection → higher SV.
        Higher viscosity → greater afterload → reduced ejection fraction.

        SV = base × volume_factor × contractility × afterload_penalty
        """
        ...
        # Afterload penalty: high viscosity → heart must push harder → SV drops
        # At normal viscosity (1.0): penalty = 1.0 (no effect)
        # At severe dehydration viscosity (2.5): penalty ≈ 0.7 (30% SV reduction)
        afterload_penalty = 1.0 / (1.0 + 0.3 * (self._blood_viscosity - VISCOSITY_NORMAL))

        sv = STROKE_VOLUME_BASE * volume_factor * self._contractility * afterload_penalty
