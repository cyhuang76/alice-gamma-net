...
from alice.brain.neural_display import NeuralActivityDisplay as ConsciousnessScreen
...
class SystemState:
    ...
    # ★ Screen brightness Φ (impedance-based consciousness)
    #   Synced from ConsciousnessModule.phi each tick.
    #   Phase 34: sole freeze signal — is_frozen() = (screen_phi < 0.15).
    #   screen_phi already includes thalamic gateway Γ cascade from
    #   clinical_neurology, so no separate bridge or AND gate needed.
    screen_phi: float | None = None   # None = not yet synced from ConsciousnessModule
    ...
    def tick(
        ...
        #   (1-p) = parasympathetic brake (p = sympathetic tone, 0~1)
        #
        # When parasympathetic is active (low p): strong cooling → temperature drops
        # When sympathetic dominant (high p): weak cooling → temperature rises
        # This is why you can't calm down during a panic attack — p→1 → brake off
        #
        sympathetic_tone = min(1.0, critical_pressure * 0.5 + emotional_stress * 0.3 + self.pain_level * 0.2)
        parasympathetic_brake = 1.0 - sympathetic_tone
        # Phase 33b fix: increased cooling (0.03→0.035) and reduced
        # sympathetic suppression (0.7→0.5) to widen thermal margin.
        # Old: cooling = 0.03 * (1.0 - sympathetic_tone * 0.7)
        #   → range [0.009, 0.03], baseline T≈0.73 → pain cascade too easy
        # New: range [0.0175, 0.035], weaker positive feedback under stress.
        cooling = 0.035 * (1.0 - sympathetic_tone * 0.5)
    ...
    def is_frozen(self) -> bool:
        """Whether the system is frozen (cognitive paralysis).

        Phase 34: single-track physics.
          Freeze decision comes from screen_phi (= screen brightness = Φ).
          screen_phi is the emergent result of ALL impedance mismatches:
            coaxial Γ + thalamic gateway Γ + clinical Γ → T_pixel → brightness.
          No Boolean AND gate needed — the physics cascades naturally.

          Falls back to vitals.consciousness only if screen_phi is not yet
          available (first tick before first render).
        """
        if self.screen_phi is not None:
            return self.screen_phi < 0.15
...
class AliceBrain:
    ...
    def __init__(self, neuron_count: int = 100):
        ...
        # ★ Consciousness screen — terminal rendering surface (TV screen model)
        #   Eye(Camera) → CoaxialChannel(Cable) → Screen(ConsciousnessScreen)
        #   Pixel clarity: T_i = 1 - Γ_i²  (impedance matching at each neuron terminal)
        self.visual_screen = ConsciousnessScreen(
            resolution=neuron_count,
            screen_id="visual",
        )
        ...
        # ★ Skin — somatosensory organ (touch, temperature, pain/nociception)
        #   Z_skin(T) = Z_base × (1 + α_T × (T - T_ref))
        #   Γ_touch = |Z_object - Z_skin| / (Z_object + Z_skin)
        #   "Touch is impedance matching at the boundary between self and world."
        self.skin = AliceSkin()

        # ★ Nose — olfactory molecular impedance sensor bank
        #   Unique: ONLY sense that bypasses the thalamus → direct amygdala pathway
        #   Binding affinity = Lorentzian resonance between odorant and receptor impedances
        #   "You stop smelling your own house because your receptors impedance-matched to it."
        self.nose = AliceNose()

        # ★ Vestibular system — inertial impedance sensor for balance
        #   Semicircular canals (angular accel) + otoliths (linear accel + gravity)
        #   Motion sickness = Γ between visual prediction and vestibular measurement
        #   "Motion sickness is an impedance mismatch between your eyes and your inner ear."
        self.vestibular = VestibularSystem()

        # ★ Interoception — internal body state monitor
        #   8-channel interoceptive network (cardiac, respiratory, gastric, thermal, ...)
        #   Body budget: predicted vs actual → Γ_intero → emotional valence
        #   "Anxiety is the impedance mismatch between what your body predicts and what it measures."
        self.interoception = InteroceptionOrgan()
        ...
        # ★ Physiology dedup guard (Phase 34b)
        #   perceive() might be called multiple times per logical tick
        #   (e.g. hear→perceive + see→perceive in a single run_tick).
        #   Autonomic, lung, cardiovascular, and sleep physiology should
        #   run ONCE per logical tick.
        #   Call begin_tick() before each hear+see group.  When begin_tick()
        #   is used, physiology runs only once.  When begin_tick() is NOT
        #   called (legacy), physiology runs on every perceive() (no change).
        self._logical_tick: int = 0       # incremented by begin_tick()
        self._last_physiology_tick: int = -1  # last logical tick physiology ran
        self._begin_tick_used: bool = False   # True once begin_tick() is called

    # ------------------------------------------------------------------
    # Consciousness screen rendering — TV terminal rendering surface
    # ------------------------------------------------------------------

    def _render_consciousness_screen(self):
        """
        Render one frame on the consciousness screen.

        Physics:
          Each neuron in the somatosensory cortex = one pixel on the visual screen.
          Pixel clarity T_i = T_channel × T_terminal
          T_channel  = 1 - Γ_ch²    (cable quality from cycle reports)
          T_terminal = 4s/(1+s)²     (neuron health from synaptic_strength)

        Thalamic gateway (Phase 34):
          The thalamus is a series impedance mismatch on ALL sensory paths to cortex.
          Signals must pass through it before reaching the consciousness screen.
          clinical_neurology.channel_gamma["thalamus"] and ["consciousness"]
          cascade with the coaxial Γ as series transmission stages:
            T_effective = T_coaxial × T_clinical
            Γ_effective = √(1 − T_effective)
          This is the same physics as two coaxial connectors in series —
          each impedance mismatch attenuates the signal independently.
        """
        # --- Neuron data from visual pathway (somatosensory cortex) ---
        region = self.fusion_brain.regions[BrainRegionType.SOMATOSENSORY]
        neurons = region.neurons
        n = min(self.visual_screen.resolution, len(neurons))
        neuron_activations = np.array([neurons[i].activation for i in range(n)])
        neuron_strengths = np.array([neurons[i].synaptic_strength for i in range(n)])

        # --- Channel Γ from last cycle's coaxial transmissions ---
        avg_gamma = 0.0
        reports = self.fusion_brain._cycle_reports
        if reports:
            avg_gamma = float(np.mean([r.reflection_coefficient for r in reports]))

        # --- Thalamic gateway: clinical Γ cascades as series impedance ---
        #   T_coaxial  = 1 − Γ_coax²
        #   T_clinical = Π(1 − Γ_k²)  for k in {thalamus, consciousness, perception}
        #   T_total    = T_coaxial × T_clinical
        #   Γ_total    = √(1 − T_total)
        #
        # This replaces the Phase 33b "bridge hack" — clinical Γ now physically
        # attenuates the screen render instead of numerically overwriting screen_phi.
        try:
            _clinical_gammas = self.clinical_neurology.get_merged_channel_gamma()
        except Exception:
            _clinical_gammas = {}
        # Also include pharmacology's merged gamma
        try:
            _pharma_gammas = getattr(self, '_last_pharma_gamma', {})
        except Exception:
            _pharma_gammas = {}

        # Gateway channels: these represent the mandatory stages in the
        # path to conscious awareness.  Each is a series impedance mismatch:
        #   thalamus     — sensory relay gateway (ALL input must pass through)
        #   consciousness — global cortical integration
        #   perception   — sensory processing fidelity
        #   attention    — global workspace selection (inattentional blindness = Γ↑)
        #   prefrontal   — executive monitoring (anesthesia suppresses this)
        T_clinical = 1.0
        for _gw_key in ("thalamus", "consciousness", "perception",
                        "attention", "prefrontal"):
            _gw_gamma = max(
                _clinical_gammas.get(_gw_key, 0.0),
                _pharma_gammas.get(_gw_key, 0.0),
            )
            if _gw_gamma > 0.0:
                T_clinical *= (1.0 - _gw_gamma ** 2)

        T_coaxial = 1.0 - avg_gamma ** 2
        T_total = T_coaxial * T_clinical
        effective_gamma = float(np.sqrt(np.clip(1.0 - T_total, 0.0, 1.0)))

        # --- Temperature coupling (Johnson-Nyquist noise) ---
        self.visual_screen.set_temperature(self.vitals.ram_temperature)

        # --- Render ---
        if self.sleep_cycle.is_sleeping():
            # Dream mode: camera off, screen renders from memory replay
            frame = self.visual_screen.render_dream(
                memory_values=neuron_activations,
                neuron_strengths=neuron_strengths,
                dream_vividness=0.4 + 0.3 * self.consciousness.phi,
            )
        else:
            frame = self.visual_screen.render(
                signal_values=neuron_activations,
                neuron_strengths=neuron_strengths,
                channel_gamma=effective_gamma,
                signal_frequency=10.0,
                pain_level=self.vitals.pain_level,
            )

        # --- PTSD burn-in: severe pain burns trauma into screen ---
        if self.vitals.pain_level > 0.7:
            self.visual_screen.burn(neuron_activations, intensity=self.vitals.pain_level)

        # --- Per-tick maintenance (burn-in decay, dead pixel enforcement) ---
        self.visual_screen.tick()
    ...
    def begin_tick(self) -> None:
        """Mark the start of a new logical cognitive tick.

        Call this ONCE before any sequence of hear()/see() calls that
        belong to the same external tick.  When used, autonomic + sleep
        physiology will run exactly once per logical tick (on the first
        perceive() call), avoiding the "double-perceive" problem where
        hear→perceive and see→perceive each update physiology.

        Backward compatible: if never called, every perceive() runs
        its own physiology (legacy behaviour).
        """
        self._logical_tick += 1
        self._begin_tick_used = True
    ...
    def perceive(
        ...
        # ★ Sync screen brightness Φ to vitals
        #   Phase 34: screen_phi is the sole freeze signal.
        #   It is set from the consciousness screen render (which already
        #   includes thalamic gateway Γ cascade).  No hack — just sync.
        _natural_phi = getattr(self.consciousness, 'phi', None)
        if _natural_phi is not None:
            self.vitals.screen_phi = _natural_phi
        ...
        # ★ Johnson-Nyquist: couple system temperature to coaxial channel noise
        #   Higher ram_temperature → more thermal noise in all signal channels
        self.fusion_brain.signal_bus.set_temperature(self.vitals.ram_temperature)
        ...
        # ★ Physiology dedup guard (Phase 34b)
        #   When begin_tick() is used, autonomic + organ + sleep physiology
        #   runs only ONCE per logical tick (first perceive() call).
        #   Subsequent perceive() calls reuse cached results.
        #   Without begin_tick(), every perceive() runs physiology (legacy).
        _run_physiology = True
        if self._begin_tick_used:
            if self._last_physiology_tick == self._logical_tick:
                _run_physiology = False
            else:
                self._last_physiology_tick = self._logical_tick
        ...
        if _run_physiology:
            ...
            # Cache results for second perceive() of this logical tick
            self._cached_lung_result = lung_result
            self._cached_cv_result = cv_result
            self._cached_sleep_info = sleep_info
            self._cached_sleep_phys = sleep_phys

        else:
            # ★ Second perceive() in same logical tick — reuse cached physiology
            lung_result = getattr(self, '_cached_lung_result', {})
            cv_result = getattr(self, '_cached_cv_result', {})
            sleep_info = getattr(self, '_cached_sleep_info', {
                "stage": self.sleep_cycle.stage.value,
                "sensory_gate": 1.0,
            ...
            sleep_phys = getattr(self, '_cached_sleep_phys', {})
        ...
        # ★ Render consciousness screen FIRST — Φ = screen brightness
        #   Screen renders the coaxial cable terminal state → brightness = Φ
        screen_frame = self._render_consciousness_screen()
        screen_brightness = screen_frame.brightness

        # ★ Consciousness update — reads screen brightness as Φ
        ...
        # 12. (Mirror neurons — classified; stub defaults for downstream)
        mirror_result = {"has_social_input": False, "empathic_valence": 0.0}

        # 12b. (Social resonance — classified; stub defaults for downstream)
        social_result = {"social_bond": 0.0, "is_lonely": False, "social_need": 0.0}
        ...
        # ★ Narrative memory closed-loop: arc coherence → semantic pressure modulation
        #   An incoherent narrative (low coherence = high Γ_causal) creates drive
        #   to "make sense of it" → pressure ↑.  A tragic arc intensifies tension.
        #   Without this, narrative is computed but has zero effect on language drive.
        _narrative_pressure_boost = 0.0
        if self.narrative_memory.arcs:
            _latest_arc = self.narrative_memory.arcs[-1]
            _narrative_incoherence = 1.0 - _latest_arc.coherence
            _narrative_pressure_boost += _narrative_incoherence * 0.1
            if _latest_arc.emotional_arc_type == "tragedy":
                _narrative_pressure_boost += 0.05
            elif _latest_arc.emotional_arc_type == "growth":
                _narrative_pressure_boost += 0.02

        # ★ Recursive grammar closed-loop: parse active concepts → structural Γ → pressure
        #   High structural Γ = "I can't form sentences" → frustration → pressure ↑.
        #   Low structural Γ = coherent syntax → fluent expression → pressure release.
        #   Without this, grammar computes rules but never affects speech production.
        _grammar_structural_gamma = 1.0
        _grammar_comprehension = 0.0
        if self.semantic_pressure.concept_sequence:
            _recent_concepts = self.semantic_pressure.concept_sequence[-8:]
            if len(_recent_concepts) >= 2:
                try:
                    _parse_result = self.recursive_grammar.parse(_recent_concepts)
                    _grammar_structural_gamma = _parse_result.gamma_structural
                    _grammar_comprehension = _parse_result.comprehension
                    grammar_result["last_parse_gamma"] = round(_grammar_structural_gamma, 4)
                    grammar_result["last_parse_comprehension"] = round(_grammar_comprehension, 4)
                except Exception:
                    pass  # Parsing failure is acceptable — concepts may not be parseable

        # Apply narrative + grammar pressure boosts before semantic_pressure tick
        self.semantic_pressure.pressure += _narrative_pressure_boost
        if _grammar_structural_gamma > 0.5:
            self.semantic_pressure.pressure += _grammar_structural_gamma * 0.03
        elif _grammar_comprehension > 0.3:
            self.semantic_pressure.pressure *= (1.0 - 0.1 * _grammar_comprehension)
        ...
        lifecycle_result = self.lifecycle.tick(
            reflected_energy=_global_gamma_sq,
            novelty_level=_novelty_input,
        )

        # 13c. ★ Bone china engine — memory consolidation during sleep
        if self.sleep_cycle.is_sleeping():
            bone_china_result = self.bone_china.tick(
                is_sleeping=True,
                sleep_stage=sleep_info.get("stage", "n1").lower(),
            )
        else:
            bone_china_result = self.bone_china.tick(is_sleeping=False, sleep_stage="wake")

        # 13d. ★ Fontanelle model — developmental impedance window
        _spec_index = float(np.mean([
            r.get_specialization_index() for r in self.pruning.regions.values()
        ])) if self.pruning else 0.5
        fontanelle_result = self.fontanelle.tick(
            specialization_index=_spec_index,
            gamma_sq_heat=_global_gamma_sq,
        )

        # 13e. ★ Gradient optimizer — variational impedance descent
        #    Compute analytic gradients for active channels and recommend Z adjustments
        _channels_for_grad = {}
        for name, region in self.pruning.regions.items():
            alive = region.alive_connections
            if alive:
                avg_z = float(np.mean([c.impedance for c in alive]))
                _channels_for_grad[name] = (avg_z, region.region_impedance)
        gradient_step, gradient_channels = self.gradient_optimizer.step(_channels_for_grad)
        gradient_result = {
            "sigma_gamma_sq": gradient_step.sigma_gamma_sq,
            "gradient_norm": gradient_step.gradient_norm,
            "converged": gradient_step.converged,
            "channels_updated": gradient_step.channels_updated,
        }

        # 13g. ★ Skin tick — somatosensory adaptation
        skin_result = self.skin.tick()

        # 13h. ★ Nose tick — olfactory adaptation and recovery
        nose_result = self.nose.tick()

        # 13i. ★ Vestibular tick — canal adaptation and sickness recovery
        vestibular_result = self.vestibular.tick()

        # 13j. ★ Interoception tick — internal body state monitoring
        #    Feed real body signals into interoceptive channels
        self.interoception.update_from_body(
            heart_rate=self.vitals.heart_rate,
            breath_rate=self.autonomic.breath_rate,
            gastric_fill=self.homeostatic_drive._digestion_buffer if hasattr(self.homeostatic_drive, '_digestion_buffer') else 0.3,
            core_temp=self.autonomic.core_temp,
            hydration=self.homeostatic_drive.hydration,
            visceral_pain=self.vitals.pain_level,
            fatigue_level=aging_signal.cognitive_impact,
            sympathetic_tone=self.autonomic.sympathetic,
        )
        interoception_result = self.interoception.tick()

        # 13k. ★ Huttenlocher curve — age-driven synaptogenesis/pruning modulation
        huttenlocher_result = self.pruning.apply_huttenlocher_trajectory(self._cycle_count)
        ...
        # ★ Consciousness screen result (already rendered above, before consciousness.tick)
        brain_result["consciousness_screen"] = {
            "brightness": round(screen_frame.brightness, 4),
            "contrast": round(screen_frame.contrast, 4),
            "dead_pixels": screen_frame.dead_pixels,
            "active_pixels": screen_frame.active_pixels,
            "burn_in_total": round(screen_frame.burn_in_total, 4),
            "is_overloaded": screen_frame.is_overloaded,
            "is_dreaming": screen_frame.is_dreaming,
            "peak": round(screen_frame.peak, 4),
            "frame_id": screen_frame.frame_id,
        }
        ...
        # ★ New subsystem results injection
        brain_result["lifecycle"] = dataclasses.asdict(lifecycle_result) if hasattr(lifecycle_result, '__dataclass_fields__') else lifecycle_result
        brain_result["bone_china"] = bone_china_result
        brain_result["fontanelle"] = dataclasses.asdict(fontanelle_result) if hasattr(fontanelle_result, '__dataclass_fields__') else fontanelle_result
        brain_result["gradient_optimizer"] = gradient_result
        brain_result["skin"] = skin_result
        brain_result["nose"] = nose_result
        brain_result["vestibular"] = vestibular_result
        ...
        _subsys_signals = {}
        for _name, _mod in [
            ("skin", self.skin), ("nose", self.nose),
            ("vestibular", self.vestibular), ("interoception", self.interoception),
            ("lifecycle", self.lifecycle), ("bone_china", self.bone_china),
            ("fontanelle", self.fontanelle), ("gradient_optimizer", self.gradient_optimizer),
        ]:
            try:
                _sig = _mod.get_signal()
                _subsys_signals[_name] = {
                    "amplitude": round(float(_sig.amplitude), 4),
                    "frequency": round(float(_sig.frequency), 4),
                    "impedance": round(float(_sig.impedance), 2),
                    "source": _sig.source,
                    "modality": _sig.modality,
                }
            except Exception:
                pass
        brain_result["subsystem_signals"] = _subsys_signals

        # ★ Closed-loop: aggregate reflected energy from new modules → vitals
        #   Reflected energy = Γ² waste heat → feeds back into ram_temperature
        #   Note: fontanelle handles its own heat internally; do NOT double-count.
        _subsys_reflected = 0.0
        if isinstance(interoception_result, dict):
            _subsys_reflected += interoception_result.get("gamma_intero", 0.0) ** 2
        if isinstance(vestibular_result, dict):
            _subsys_reflected += vestibular_result.get("gamma_conflict", 0.0) ** 2
        if _subsys_reflected > 0.01:
            self.vitals.ram_temperature = min(
                1.0,
                self.vitals.ram_temperature + _subsys_reflected * 0.005,
            )
        ...
        # ★ ClinicalNeurology closed-loop: merged_channel_gamma → vitals
        #   Neurological disease Γ directly impairs consciousness & motor output.
        #   Stroke → consciousness↓; ALS → motor↓; respiratory failure → acute pain.
        #   Without this, stroke/ALS/dementia are computed but have zero systemic effect.
        _neuro_gamma = clinical_result.get("merged_channel_gamma", {})
        if _neuro_gamma:
            # consciousness channel Γ → reduce vitals.consciousness
            _c_gamma = _neuro_gamma.get("consciousness", 0.0)
            if _c_gamma > 0.05:
                self.vitals.consciousness = max(
                    0.05,
                    self.vitals.consciousness * (1.0 - _c_gamma * 0.5),
                )
                # Phase 34: clinical Γ → screen_phi propagation is handled
                # by the thalamic gateway in _render_consciousness_screen().
                # No bridge hack needed — physics cascades naturally.
            # motor channel Γ → heat (motor struggle)
            _motor_gamma = max(
                _neuro_gamma.get("hand", 0.0),
                _neuro_gamma.get("motor_gross", 0.0),
            )
            if _motor_gamma > 0.1:
                self.vitals.ram_temperature = min(
                    1.0,
                    self.vitals.ram_temperature + _motor_gamma * 0.02,
                )
            # ALS respiratory failure → acute pain
            if clinical_result.get("als", {}).get("respiratory_failure", False):
                self.vitals.pain_level = min(1.0, self.vitals.pain_level + 0.3)
        ...
        # ★ Pharmacology closed-loop: merged_gamma → vitals + consciousness
        #   Drug effects physically modify channel impedance — must feed back.
        #   Therapeutic drugs lower Γ (help); side effects raise Γ (harm).
        #   Seizure → acute temperature spike. Depression → sympathetic modulation.
        #   Without this, drugs are computed but have zero therapeutic or side effect.
        _pharma_gamma = pharma_result.get("merged_gamma", {})
        # Store for thalamic gateway cascade in _render_consciousness_screen()
        self._last_pharma_gamma = _pharma_gamma
        if _pharma_gamma:
            _p_c_gamma = _pharma_gamma.get("consciousness", 0.0)
            if _p_c_gamma > 0.05:
                self.vitals.consciousness = max(
                    0.05,
                    self.vitals.consciousness * (1.0 - _p_c_gamma * 0.3),
                )
                # Phase 34: pharmacology Γ → screen_phi propagation is handled
                # by the thalamic gateway in _render_consciousness_screen().
                # Store for next render cycle's gateway cascade.
                pass
            _p_auto_gamma = _pharma_gamma.get("autonomic", 0.0)
            if _p_auto_gamma > 0.1:
                self.vitals.ram_temperature = min(
                    1.0,
                    self.vitals.ram_temperature + _p_auto_gamma * 0.015,
                )
        # Seizure → acute temperature spike
        if pharma_result.get("epilepsy", {}).get("in_seizure", False):
            self.vitals.ram_temperature = min(
                1.0,
                self.vitals.ram_temperature + 0.15,
            )
    ...
    def introspect(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "subsystems": {
                ...
                "lifecycle": self.lifecycle.get_state(),
                "bone_china": self.bone_china.get_state(),
                "fontanelle": self.fontanelle.get_state(),
                "gradient_optimizer": self.gradient_optimizer.get_stats(),
                "skin": self.skin.get_stats(),
                "nose": self.nose.get_stats(),
                "vestibular": self.vestibular.get_stats(),
                "interoception": self.interoception.get_stats(),
                "huttenlocher": self.pruning.get_huttenlocher_curve_data(),
                "consciousness_screen": self.visual_screen.get_stats(),
            },
