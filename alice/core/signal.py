...
Physical Formulas:
  ...
  Thermal noise           N = k_B_alice × T_ram × Δf × L × (1 + Γ²)
  (Johnson-Nyquist: noise power coupled to system temperature)
...
class ElectricalSignal:
    ...
    def from_raw(
        ...
        # Frequency: estimated from autocorrelation dominant period
        # Physical basis: autocorrelation finds the dominant period T of the
        # (possibly complex, multi-harmonic) signal by detecting the first
        # significant peak after lag 0. This is more robust than zero-crossing
        # rate for signals with harmonics or noise.
        # Complexity: O(N) via direct lag search (no FFT needed).
        if n > 2:
            ...
            if var > 1e-12:
                # Search for first positive peak in autocorrelation past lag 0
                # We only need to scan up to n//2 (Nyquist)
                half = max(2, n // 2)
                best_lag = 0
                best_acf = -1.0
                prev_acf = var  # lag=0 is always highest
                # Find first peak after the initial descent
                descending = True
                for lag in range(1, half):
                    # Direct dot product at this lag — O(N) per lag but we
                    # short-circuit once we find the first peak
                    acf = float(np.dot(centered[:n - lag], centered[lag:]))
                    if descending:
                        if acf > prev_acf:
                            descending = False  # now ascending toward peak
                    else:
                        if acf < prev_acf and prev_acf > 0.1 * var:
                            # prev_acf was a peak, and it's significant (>10% of var)
                            best_lag = lag - 1
                            best_acf = prev_acf
                            break
                    prev_acf = acf
                if best_lag > 0:
                    # Dominant period T ≈ best_lag samples
                    # Assume sample rate = n samples/sec (normalized)
                    estimated_freq = float(n) / float(best_lag)
                else:
                    # No clear periodicity → fall back to zero-crossing rate
                    zc = int(np.sum(np.abs(np.diff(np.sign(centered))) > 0))
                    estimated_freq = max(0.5, zc / 2.0)
            else:
                estimated_freq = 0.5  # DC signal
            # Clamp to brainwave range (0.5–100 Hz)
...
class CoaxialChannel:
    ...
    temperature: float = 0.3  # System temperature T_ram (normalized 0~1)

    # Johnson-Nyquist thermal noise constant (nondimensionalized Boltzmann)
    # Calibrated so at T=0.3 → noise ≈ 0.001 (backward-compatible),
    # at T=0.9 → noise ≈ 3× higher (signal degradation onset)
    _K_B_ALICE: ClassVar[float] = 0.0033
    ...
    def transmit(self, signal: ElectricalSignal) -> Tuple["ElectricalSignal", "TransmissionReport"]:
        ...
        # === 3. Bandwidth Limitation (2nd-order Butterworth filter) ===
        # H(f) = 1 / sqrt(1 + (f/f_c)^(2n)), n=2 (second order)
        # Physical basis: real coaxial cables have smooth roll-off beyond cutoff,
        # not a hard linear wall. A 2nd-order Butterworth provides maximally-flat
        # passband with ~40dB/decade roll-off — standard RF engineering model.
        f_ratio = signal.frequency / max(1.0, self.bandwidth)
        bandwidth_factor = 1.0 / math.sqrt(1.0 + f_ratio ** 4)  # n=2 → exponent=2n=4
        bandwidth_factor = max(0.1, bandwidth_factor)  # floor: never fully zero
        ...
        # N = k_B_alice × T_ram × Δf_eff × L × (1 + Γ²)
        # Physical basis: noise power ∝ absolute temperature (Johnson 1928)
        # At T=0.3 (normal): noise ≈ 0.001 (gentle perturbation, SNR ~25dB)
        # At T=0.9 (fever):  noise ≈ 0.003 (signal degradation begins)
        effective_bandwidth = min(signal.frequency, self.bandwidth)
        noise_level = (
            self._K_B_ALICE
            * max(0.01, self.temperature)  # floor to avoid zero noise
            * (effective_bandwidth / 10.0)  # normalize to 10Hz reference
            * self.length
            * (1.0 + reflected_ratio)
        )
        noise_level = min(noise_level, 0.05)  # safety cap: prevent signal avalanche
...
class SignalBus:
    ...
    def set_temperature(self, temperature: float) -> None:
        """
        Update thermal temperature for all channels (Johnson-Nyquist coupling).

        Called each tick by AliceBrain with vitals.ram_temperature.
        Higher temperature → all channels generate more thermal noise.
        """
        t = max(0.01, min(1.0, temperature))
