...
class AutonomicNervousSystem:
    ...
    def tick(
        ...
        # Respiration
        if voluntary_breath is not None:
            target_br = float(np.clip(voluntary_breath, MIN_BREATH_RATE, MAX_BREATH_RATE))
        ...
        self.breath_rate = float(np.clip(self.breath_rate, MIN_BREATH_RATE, MAX_BREATH_RATE))
    ...
    def get_signal(self) -> ElectricalSignal:
        ...
        br_norm = (self.breath_rate - MIN_BREATH_RATE) / (MAX_BREATH_RATE - MIN_BREATH_RATE)
