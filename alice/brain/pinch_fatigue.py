...
# Physical basis: yield strength drops exponentially with temperature
# σ_yield(T) = σ_yield(T_ref) / exp(E_a(T - T_ref) / T_ref²)
# Nondimensionalized: E_a_pinch calibrated so T=0.8 → yield drops ~25%
ARRHENIUS_PINCH_EA = 3.0            # Effective activation energy for pinch fatigue
...
class PinchFatigueEngine:
    ...
    def apply_current_pulse(
        ...
        # Physical: σ_yield ∝ exp(-E_a(T-T_ref)/T_ref²) (Arrhenius thermal softening)
        # At T=T_ref(0.5): factor=1.0; T=0.8: factor≈1.25 (yield drops 20%)
        temp_delta = temperature - REFERENCE_TEMP
        temp_factor = math.exp(
            ARRHENIUS_PINCH_EA * temp_delta / max(REFERENCE_TEMP ** 2, 0.04)
        )
        temp_factor = min(temp_factor, 5.0)  # safety cap: prevent runaway
