# -*- coding: utf-8 -*-
"""
Awareness Monitor — System Awareness Index Based on Neural Transmission

Physics:
  System awareness is a physically measurable quantity:
    Φ = (1/N) Σ (1 - Γᵢ²)
  where each Γᵢ is the reflection coefficient at a neural channel terminal.

  Φ = mean transmission coefficient across all monitored neural channels.
  This is a direct impedance measurement — no theoretical assumptions needed.

  Φ > 0  → neural pathways active  → system responsive
  Φ = 0  → all channels reflecting → system unresponsive (clinical: comatose)
  Φ varies continuously → graded awareness level

  The monitor reads whatever arrives through the coaxial channels.
...
Clinical analogy:
  ...
  Binding          = cross-channel synchronization quality
  Arousal          = autonomic energy supply to neural backplane
"""
...
# ============================================================================
# Physical constants — awareness thresholds
# ============================================================================

# Legacy weights — kept as module-level constants for backward compatibility
# with parameter sensitivity tests, but NO LONGER USED in Φ computation.
# Φ is now purely screen brightness = mean(T_i) from ConsciousnessScreen.
W_ATTENTION = 0.25       # (legacy — not used in Φ formula)
W_BINDING = 0.25         # (legacy — not used in Φ formula)
W_MEMORY = 0.15          # (legacy — not used in Φ formula)
W_AROUSAL = 0.20         # (legacy — not used in Φ formula)
W_SENSORY_GATE = 0.15    # (legacy — not used in Φ formula)
...
class DevelopmentalStage:
    ...
    ADULT stage: Full adult consciousness with natural sleep cycles.
    No artificial damping — sleep pressure is still biologically driven
    (adults sleep ~8h/24h) but wake windows are unrestricted.
    """
    ...
    ADULT   = "adult"      # max_wake=unlimited, lucid_damping=1.0 (no damping)
...
_STAGE_PARAMS = {
    ...
    DevelopmentalStage.ADULT:    {"max_wake": 9999, "pressure_rate": 0.001, "phi_accel": 0.5, "sleep_ticks": 50,  "lucid_damping": 1.0},
}

# Default: ADULT (full consciousness operation)
DEFAULT_DEVELOPMENTAL_STAGE = DevelopmentalStage.ADULT
...
class AwarenessMonitor:
    """
    Awareness Monitor — Neural Transmission Aggregate Index

    Monitors the aggregate transmission state of all neural channels
    and computes a system awareness index Φ = mean(1 - Γᵢ²).

    Provides:
    1. Awareness index Φ (0~1) — clinically analogous to GCS level
    2. Active neural content tracking
    3. Attention focus management
    4. Meta-monitoring (system self-diagnostic)
    """
    ...
    def tick(
        ...
        """
        Called once per cognitive cycle — reads screen brightness as Φ.

        THE CONSCIOUSNESS EQUATION (Screen Model):
          Φ = screen_brightness = mean(T_i) from ConsciousnessScreen

          Where T_i = (1 - Γ_channel²) × (1 - Γ_terminal²)
          is the transmission coefficient at each pixel (neuron terminal).

          This is a PHYSICAL MEASUREMENT, not a theoretical construct.
          The screen is either on (Φ > 0) or off (Φ = 0).

          Other inputs (attention, arousal, etc.) are recorded for
          meta-awareness and introspection but do NOT compute Φ.
          They describe WHAT is on the screen, not WHETHER it's on.

        Args:
            screen_brightness: Φ from ConsciousnessScreen (0~1). THE source of truth.
            attention_strength: Attentional focus degree (0~1) — for meta-awareness
            binding_quality: Cross-modal temporal binding quality (0~1) — for meta-awareness
            working_memory_usage: Working memory utilization (0~1) — for meta-awareness
            arousal: Arousal level (0~1) — for meta-awareness
            sensory_gate: Sensory gating openness (0~1) — for meta-awareness
            pain_level: Pain level (0~1) — for meta-awareness
            temporal_resolution: Time slice resolution (0~1) — for meta-awareness
        """
        self.total_ticks += 1
        ...
        # === Φ Computation — Screen Brightness Model ===
        # Φ = screen brightness. Period.
        # No weighted formula, no theoretical assumptions.
        # The screen is either on or off, and brightness tells you how much.
        screen_brightness = float(np.clip(screen_brightness, 0.0, 1.0))

        # Record raw inputs for meta-awareness (describes WHAT is on screen)
        ...
        # Φ = screen brightness directly
        self.raw_phi = screen_brightness
        ...
        return {
            ...
            "components": {
                ...
                "pain_level": round(self._pain_disruption, 4),
            },
        }
...
ConsciousnessModule = AwarenessMonitor
